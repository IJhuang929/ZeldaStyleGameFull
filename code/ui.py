...
class UI:
    def __init__(self):
        ...
        )  # (left,top,width,height) in pixel
    ...
    def show_exp(self, exp):
        text_surf = self.font.render(
            str(int(exp)), False, TEXT_COLOR
        )  # (info, false, color)
        x = self.display_surface.get_size()[0] - 20
        y = self.display_surface.get_size()[1] - 20
        text_rect = text_surf.get_rect(bottomright=(x, y))
        # bottom right exp format(box and frame)
    ...
    def weapon_overlay(self, weapon_index, has_switched):
        bg_rect = self.selection_box(10, 630, has_switched)  # for weapon display place
    ...
    def magic_overlay(self, magic_index, has_switched):
        bg_rect = self.selection_box(80, 635, has_switched)  # for magic display place
    ...
    def display(self, player):
        ...
        # let weapon and magic change active in the first second(frame turn gold)
